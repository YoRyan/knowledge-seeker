...
def populate_episode(season, episode, key, cur):
    ...
    while success:
        if last is None or significant_frame(image, last):
            ...
            big_scale = current_app.config['JPEG_VRES']/image.shape[1]
            big_image = cv2.resize(
                image,
                (round(image.shape[0]*big_scale), round(image.shape[1]*big_scale)),
            ...
            tiny_scale = current_app.config['JPEG_TINY_VRES']/image.shape[1]
            tiny_image = cv2.resize(
                image,
                (round(image.shape[0]*tiny_scale), round(image.shape[1]*tiny_scale)),
                interpolation=cv2.INTER_AREA)
